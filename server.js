...
app.get('/stream/:type/:id.json', (req, res) => {
    ...
    const streams = [
        // EJEMPLO: Si usaras Consumet u otra API que de link directo
        { 
            title: '🚀 REPRODUCTOR NATIVO (Link Directo)', 
            // En lugar de un embed, aquí iría el link al .m3u8 o .mp4
        ...
        if (type !== 'all') {
            ...
            genreBar = `
                <div class="flex gap-3 overflow-x-auto pb-4 no-scrollbar mb-6">
                    <a href="/?type=${type}" class="whitespace-nowrap px-4 py-1.5 rounded-full text-xs font-bold uppercase transition ${!genreId ? 'bg-indigo-600' : 'bg-gray-700 hover:bg-gray-700 shadow-lg shadow-indigo-500/20'}">Todos</a>
                    ${list.map(g => `
...
app.get('/search', async (req, res) => {
    ...
    try {
        // Petición a RapidAPI (Movie Database Alternative)
            ...
            <div class="grid md:grid-cols-3 gap-8">
                <div class="md:col-span-2">
                    ...
                    <div class="video-aspect bg-black rounded-xl overflow-hidden shadow-2xl">
                        <iframe id="player" src="${vidsrcUrl}" allowfullscreen frameborder="0" referrerpolicy="no-referrer" allow="autoplay; encrypted-media" sandbox="allow-forms allow-pointer-lock allow-same-origin allow-scripts allow-presentation allow-popups allow-top-navigation"></iframe>
                    </div>
                    <script>
...
// Exportar de forma compatible para Vercel (default) y Netlify (handler)
module.exports = app;
